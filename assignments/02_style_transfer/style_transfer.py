...
class StyleTransfer(object):
    def __init__(self, content_img, style_img, img_width, img_height):
        ...
        ###############################
        ## these parameters are from the paper in the begining.  
    ...
    def _content_loss(self, P, F):
        ...
        '''
        ###############################
        ## np.array supports len(), size and shape, np.list just len()
    ...
    def _single_style_loss(self, a, g):
        ...
        ###############################
        ## TO DO
        N = a.shape[3]   #numbers of filters
    ...
    def _style_loss(self, A):
        ...
        """
        n_layers = len(A)       #here A is class list, len() returns the first dime of A
    ...
    def losses(self):
        with tf.variable_scope('losses') as scope:
            with tf.Session() as sess:
                ...
                sess.run(self.input_img.assign(self.content_img))   # assign is a method of Variables class 
                gen_img_content = getattr(self.vgg, self.content_layer) #get the content layers features
                content_img_content = sess.run(gen_img_content)
            self._content_loss(content_img_content, gen_img_content)# !!! here is confusion.
    ...
    #according to build(), we are easier to know the whole procedure. Otherwise we are confused with so many self.
    def build(self):
        self.create_input()
        self.load_vgg()
        self.losses()
    ...
    def train(self, n_iters):
        ...
        with tf.Session() as sess:
            ...
            writer = tf.summary.FileWriter('graphs/style_tranfer', sess.graph)
            ...
            for index in range(initial_step, n_iters):
                ...
                if (index + 1) % skip_step == 0:
                    ...
                    if (index + 1) % 20 == 0:
                        ...
                        ###############################
                        saver.save(sess, 'checkpoints/style_stransfer/style_transfer', index)

if __name__ == '__main__':
    setup()
    machine = StyleTransfer('content/deadpool.jpeg', 'styles/picasso2.jpeg', 333, 250)
    machine.build()
    machine.train(300)
